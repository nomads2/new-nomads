...
float thoughtLifespan = 130.0; //shouldn't be more than 255.0, shouldn't be less than 40.0
String globalText = "";
...
void draw() { 
  ...
  pushMatrix();
    ...
    popMatrix();
      ...
      if (ts.getSize() <= 10) {
        thoughtLifespan = 130.0;
      } else if (ts.getSize() <= 20) {
        thoughtLifespan = 100.0;
      } else if (ts.getSize() <= 25) {
        thoughtLifespan = 70.0;
      } else if (ts.getSize() <= 30) {
        thoughtLifespan = 50.0;
      }
